import numpy
import pandas
df=pandas.read_table("D:\\beijingshishibingbaoshujuji.txt",' ')
f=open('D:\\bingbaobufenceng1mingcheng.txt','r')
s=f.readlines()
...
f1=open('D:\\bufencengbingbaoZHENGHE.txt','w',encoding='utf-8')
...
for i in df.index:
    for j in range(0,len(s)):
        if(duiyingbufencengwenjianmingchen[i].strip()==s[j].strip()):
            f2=open(str('D:\\bingbaobufenceng3\\')+str(s[j]).strip(),'r')
            b2=f2.readlines()
