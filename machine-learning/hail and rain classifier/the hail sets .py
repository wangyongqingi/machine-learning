...
import numpy
import pandas
df=pandas.read_table("D:\\prediction of storm and rain\\data sets\\beijingshishibingbaoshujuji.txt",' ')
f=open('D:\prediction of storm and rain\data sets\\BINGBAO3MINGCHENG.txt','r')
s=f.readlines()
zhoubianhour=[0 for x in range(0,len(df))]
zhoubianday=[0 for x in range(0,len(df))]
zhoubianyear=[0 for x in range(0,len(df))]
zhoubianmon=[0 for x in range(0,len(df))]
zhoubianyearmondayhour=[0 for x in range(0,len(df))]
duiyingbufencengwenjianmingchen=[0 for x in range(0,len(df))]
for i in df.index:
...
f1=open('D:\prediction of storm and rain\data sets\\hail sets.txt','w',encoding='utf-8')
...
for i in df.index:
    for j in range(0,len(s)):
        if(duiyingbufencengwenjianmingchen[i].strip()==s[j].strip()):
            f2=open(str('D:\\prediction of storm and rain\\data sets\\bufenceng3\\')+str(s[j]).strip(),'r')
            b2=f2.readlines()
