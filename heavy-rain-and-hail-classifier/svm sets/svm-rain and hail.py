# -*- coding: utf-8 -*-
"""
Created on Wed May  9 11:12:35 2018
...
dfa=pandas.read_table('D:\\prediction of storm and rain\\svm-rain and hail\\svm sets\\svmpredict1.txt',' ')
df=pandas.read_table('D:\\prediction of storm and rain\\svm-rain and hail\\svm sets\\svmtrain1.txt',' ')
...
a=str1.split()
dflist=[[0 for col in range(len(a))] for row in range(len(df))]
...
dfalist=[[0 for col in range(len(a))] for row in range(len(dfa))]
for i in range(0,len(dfa)):
    for j in range(0,len(a)):
        dfalist[i][j]=dfa[str(a[j])][i]
predict=0
for i in range(0,len(dfa)):
    if((i<10)and(clf.predict(dfalist[i])==1)):
        predict+=1
    if((i>=10)and(clf.predict(dfalist[i])==-1)):
        predict+=1
print(str('精确度：')+' '+str(predict/70))
